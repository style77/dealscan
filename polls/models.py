    ...
    def clean(self):
        if self.default_answers and self.use_ratings and self.allow_custom_answer:
            raise ValidationError(
                _("Poll cannot have default answers and allow custom answers with ratings enabled.")
            )
        super().clean()
...
class PollAnswer(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="poll"
    )
